...
def create_blueprint(MakeSession: sessionmaker):
    ...
    def getTransactions():
        ...
        transactionHistory = select(
            t_transactions.columns.company_id,
            t_transactions.columns.num_shares,
            t_transactions.columns.buy_or_sell,
            t_transactions.columns.time_executed,
        ).filter(t_transactions.columns.user_id == numb)
